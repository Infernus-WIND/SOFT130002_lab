...
function showWindowHref(){
    let urlBack = url.value.split("?");
    let nameValuePairs = urlBack[1].split("&");
    for (let i = 0;i < nameValuePairs.length;i++){
        let pair = nameValuePairs[i].split("=");
        if (pair[0] === "name"){
...
function arrSameStr(){
    ...
    for (let k in newObj) {
        if (newObj[k] > mostObj.mostValue) {
            mostObj.mostValue = newObj[k];
            mostObj.mostChar = k;
        }
    }
    result.value = "The most character is:" + mostObj.mostChar + " times:" + mostObj.mostValue ;
}
